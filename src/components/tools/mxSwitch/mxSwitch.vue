<template>
  <mx-dialog v-if="showDialog" :cmd="cmd" :cell="cell" @close="showDialog=false"></mx-dialog>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      y: null,
      showDialog: false,
      cmd: {style: { width: 0, height: 0 }, name: "triggerSetting", title: "Trigger Setting"},
    }
  },
  methods: {
    ...
    addSidebarIcon: function(graph, sidebar, ctx, image, type) {
      ...
      let funct = function(graph, evt, cell, x, y) {
        ...
        const ComponentClass = Vue.extend({
          ...
          template: '<select-switch :message-handler="messageHandler"></select-switch>',
        });
    ...
    openTriggerSetting: function(menu, cell, evt, cx) {
      cx.cmd.style = { width: 600, height: 400 };
      cx.cmd.name = "triggerSetting";
      cx.cmd.title = "Trigger Setting";
      cx.showDialog = true;
    },
    openPushSetting: function(menu, cell, evt, cx) {
      cx.cmd.style = { width: 600, height: 300 };
      cx.cmd.name = "pushSetting";
      cx.cmd.title = "Push Setting";
      cx.showDialog = true;
    },
    ...
    graph.convertValueToString = function(cell) {
      if (this.model.isVertex(cell) && cell.getType() === "switch") {
       if (typeof cell.vueComponent === 'undefined'){
         cell.vueComponent = that.$store.getters.getVueComponentByObject(that, cell, cell.getData().config.switchName);
       }
