...
import HighchartsLine from "./components/line/highcharts_line/HighchartsLine";
...
export default {
  ...
  created() {
    ...
    this.$store.commit("ADD_VUE_OBJECT", {
      name: "vue_HighchartsLine",
      components: {
        HighchartsLine
      },
      props: ['cell', 'store'],
      template: '<highcharts-line :styles="cStyle" :cell="cell" :store="store"></highcharts-line>',
      computed: {
        cStyle() {
          ...
          mxUtils.bind(this, function(evt) {
            ...
            that.colorPicker = new Picker({
              ...
              onDone: mxUtils.bind(this, function(color) {
                ...
                this.state.cell.style = this.state.cell.getType() + ';fillColor=' + that.fillColor;
...
*/
    ...
    mxVertexToolHandler.prototype.redraw = function() {
      ...
      this.redrawTools();
      if (this.state.cell != null && this.state.cell.vueComponent) {
        let el = this.state.cell.vueComponent.$el;
        let geo = this.state.cell.geometry;
        that.console.dir(geo);
        let rect = el.parentElement.parentElement.parentElement.parentElement.previousElementSibling.querySelector("rect");
        switch (this.state.cell.type) {
          case "chart":
            el.parentElement.parentElement.setAttribute("width", rect.getAttribute("width"));
            el.parentElement.parentElement.setAttribute("height", rect.getAttribute("height"));
            el.parentElement.parentElement.parentElement.setAttribute("transform",`translate(${rect.getAttribute("x")}, ${rect.getAttribute("y")})`);
            break;
        }
      }
    };
